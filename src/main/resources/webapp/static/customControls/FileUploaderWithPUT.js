sap.ui.define([
	...
	return FileUploader.extend("co.za.tourdeforce.TDF_app.customControls.FileUploaderWithPUT", {
		metadata: {
			properties: {
				"method": {
					type: "string"
		...
		_sendFilesWithXHR: function (aFiles) {
			...
			if (aFiles.length > 0) {
				...
				for (var j = 0; j < iFiles; j++) {
					...
					oXhrEntry.xhr.open(this.getProperty("method"), this.getUploadUrl(), true);
					if (oXHRSettings) {
